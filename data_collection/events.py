...
class Events:
    ...
    def task_summary(self) -> dict:
        ...
        return {
            ...
            # The full, detailed stats from the WebSocketManager are included here.
    ...
    async def start(self, markets_exhausted: asyncio.Event, restore = False):
        ...
        # If registry is empty, set up bootstrap and wait for the first token.
        ...
        try:
            ...
            # Populate initial subscriptions from existing tokens
            ...
            # Start background serializer task
    ...
    async def _restore_subscriptions_from_db(self):
        ...
        try:
            results = await self._db_cli.fetch(name="get_all_active_asset_ids", stmt=stmt)
            if not results:
                self._log("No existing active tokens found in database to restore.", "INFO")
                return
            
            self._log(f"Found {len(results)} active tokens in database to restore and subscribe to.", "INFO")
            for record in results:
                asset_id = record.get("asset_id")
                if isinstance(asset_id, str):
                    # This will correctly add only active tokens to the subscription batches.
                    self._orderbook_registry.insert_token(asset_id)
            self._log("Finished inserting restored tokens for subscription.", "INFO")
        except Exception as e:
    ...
    def _bootstrap(self, token: str):
        # This is called only on the very first token insert.
        if self._bootstrap_callback_handle is not None:
            self._log(f"bootstrap: First token '{token}' received, starting main module.", "INFO")
            self._orderbook_registry.unregister_insert_callback(self._bootstrap_callback_handle)
            self._bootstrap_callback_handle = None
            # Create a task to avoid blocking the callback dispatcher
    ...
    def _token_delete_callback(self, token: str)->bool:
        ...
        if self._orderbook_registry.get_token_count() == 0:
            ...
            self.stop()
            # Re-arm the bootstrap mechanism
            if self._bootstrap_callback_handle is None:
                self._bootstrap_callback_handle = self._orderbook_registry.register_insert_callback(self._bootstrap)
            return
    ...
    async def _serializer(self):
        while True:
            try:
                ...
                old_subscription_count = len(self._subscriptions) + 1
                    
                # Rebuild all subscription payloads from scratch
                self._log(f"serializer: going from self._payl")
                self._new_batch.clear()
                self._subscriptions.clear()
                self._orderbook_registry.iterate_over_tokens(self._serialize)
                
                # Start new WebSocket tasks for the rebuilt subscriptions
                ...
                self._start_subscription_task(new_subscription_count, final_payload)
