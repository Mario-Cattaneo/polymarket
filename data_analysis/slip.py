...
def plot_usdc_to_slippage_ratio(ratio_data: dict):
    ...
    ax.plot(df['timestamp'], df['ratio'], 
           color='darkgreen', linewidth=2.5, label='USDC Inflow / Slippage')
    
    ax.fill_between(df['timestamp'], df['ratio'], 
                    alpha=0.3, color='darkgreen')
    
    ax.set_xlabel('Timestamp', fontsize=14)
    ax.set_ylabel('Cumulative Taker USDC Inflow / Cumulative Slippage (log scale)', fontsize=14)
    ax.set_title('Cumulative Taker USDC Inflow Divided by Cumulative Slippage', fontsize=16)
    ax.set_yscale('log')
    ax.legend(fontsize=12)
