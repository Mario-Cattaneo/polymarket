...
RTT_INSERT_STMT = "INSERT INTO markets_rtt (timestamp_ms, rtt) VALUES ($1, $2) ON CONFLICT (timestamp_ms) DO NOTHING;"
...
class Markets:
    def __init__(self, log: callable, http_man: HttpManager, db_man: DatabaseManager, orderbook_reg : OrderbookRegistry):
        ...
        self._new_markets_rows: List[Tuple] = []
        self._new_markets_flag = asyncio.Event()
        ...
        # This initialization is correct IF the method signatures below are also correct.
    ...
    async def start(self, restore: bool = False):
        try:
            ...
            self._running_db_tasks[MARKET_INSERTER_ID] = self._db_cli.exec_persistent(
                task_id=MARKET_INSERTER_ID, stmt=MARKETS_INSERT_STMT, params_buffer=self._new_markets_rows,
                signal=self._new_markets_flag, on_success=self._on_insert_success, on_failure=self._on_inserter_failure
            )
    ...
    # Signature Check: 0 parameters (self is ignored) -> Correct for http_cli
    ...
    # Signature Check: 5 parameters (self is ignored) -> Correct for http_cli
    async def _on_response(self, request_id: int, response_content: ResponseContent, status_code: StatusCode, headers: Optional[Dict[str, str]],  rtt: RTT) -> bool:
        ...
        for market_obj in markets:
            ...
            if not isinstance(token_ids_str, str):
                self._log(f"on_response: request_id {request_id} invalid 'clobTokenIds' {token_ids_str}", "WARNING")
                continue
            ...
            for token in tokens:
                self._new_markets_rows.append((now_ms, token, self._exhaustion_cycle, market_id, market_obj.get("conditionId"), market_obj.get("negRiskMarketID"), request_id, orjson.dumps(market_obj).decode('utf-8')))
    ...
    # Signature Check: 2 parameters (self is ignored) -> Correct for http_cli
    ...
    async def _on_insert_success(self, task_id: str, params: List[Tuple]):
        ...
        if task_id == MARKET_INSERTER_ID:
            self._new_markets_rows.clear()
