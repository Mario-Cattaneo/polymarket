...
SETUP_SCHEMA_STMT = """
    ...
    CREATE TABLE IF NOT EXISTS books (
        ...
        message jsonb NOT NULL,
        PRIMARY KEY (asset_id, server_time)
    );
    ...
    CREATE TABLE IF NOT EXISTS price_changes (
        ...
        conflict_count BIGINT NOT NULL DEFAULT 1,
        message jsonb NOT NULL,
        PRIMARY KEY (asset_id, server_time, side, price)
    );
    ...
    CREATE TABLE IF NOT EXISTS events_connections(
        ...
        conflict_count BIGINT NOT NULL DEFAULT 1,
        CONSTRAINT pk_events_connections PRIMARY KEY (success, timestamp_ms)
    );
"""
...
# --- System Monitoring ---
monitor_sleep = 2
# Global set to track tasks between monitoring cycles
previous_tasks = set()
...
def _format_task_info(task: asyncio.Task) -> str:
    ...
    try:
        name = task.get_name()
        # The default name is 'Task-X', which isn't very helpful.
        # Let's try to get more info from the coroutine.
        if name.startswith('Task-'):
             coro = task.get_coro()
             if hasattr(coro, '__qualname__'):
                 name = f"{name}: {coro.__qualname__}"
        return name
    except Exception:
...
async def monitor_system(markets, analytics, events):
    """A unified monitoring task that logs high-level status, memory, and task lifecycle."""
    ...
    try:
        ...
        if new_tasks:
            log_content += f"--- New Tasks ---\n{new_tasks_info}\n\n"
        if finished_tasks:
            log_content += f"--- Finished Tasks ---\n{finished_tasks_info}\n\n"
